...
function ev(d,p,section,snippet){
  return pageEvidence({...p,doc:d.name,title:d.title,section:section||p.section},snippet||p.text);
}
function blockBetween(text,startRe,endRe){
  const f=flat(text), sm=f.match(startRe); if(!sm)return '';
  const s=sm.index+sm[0].length, rest=f.slice(s), em=rest.match(endRe);
  return clean(em?rest.slice(0,em.index):rest);
}
function findEvidence(d,re,section){
  for(const p of d.pages){const f=flat(p.text),m=f.match(re);if(m)return ev(d,p,section,around(f,m.index,850))}
  return null;
}
function shareRange(raw){
  const nums=(String(raw).match(/\d+(?:\.\d+)?/g)||[]).map(Number);
  return {raw:String(raw).replace(/\s+/g,''),lower:nums[0]||0,upper:nums.length>1?nums[1]:(nums[0]||0)};
}
function extractCompanyList(d){
  const p=d.pages.find(p=>/What.?s Included/i.test(flat(p.text))&&/Companies/i.test(flat(p.text)))||d.pages[1]||d.pages[0];
  if(!p)return [];
  ...
  for(const line of lines){
    ...
    if(n && n.length<60 && /^[A-Z0-9][A-Za-z0-9&.'’()\- ]+$/.test(n) && !/Industry|Software Publishers|Developing|Selling/i.test(n))out.push(n);
  }
  return [...new Set(out)];
}
function extractMarketShares(d,companyList){
  const out=[];
  for(const p of d.pages){
    const f=flat(p.text);
    if(!/Company Market Share \(%\)|Industry Market Share by Company/i.test(f))continue;
    for(const name of companyList){
      const re=new RegExp("\\b"+rxesc(name)+"\\s+(\\d+(?:\\.\\d+)?(?:\\s*[–—-]\\s*\\d+(?:\\.\\d+)?)?)\\s+(?:10,000\\+|5,001-10,000|1,000-5,000|[0-9,]+)","i");
      const m=f.match(re); if(!m)continue;
      const rg=shareRange(m[1]);
      out.push({name:name,share:rg.raw+'%',lower:rg.lower,upper:rg.upper,evidence:ev(d,p,'Companies / Market Share',around(f,m.index,500))});
    }
  }
  return [...new Map(out.map(x=>[x.name.toLowerCase(),x])).values()];
}
function extractAtAGlance(d){
  const p=d.pages.find(p=>/At a Glance/i.test(flat(p.text))&&/Major Players/i.test(flat(p.text))&&/Revenue\s+\$?\s*\d/i.test(flat(p.text)));
  if(!p)return null;
  const f=flat(p.text);
  ...
  const wages=f.match(/Wages\s+\$?\s*(\d+(?:\.\d+)?)\s*(bn|billion|m|million)/i);
  return {
    revenue:rev?('
...
function runPipeline(){
  ...
  const sel=selectPrimary(company);

  if(!sel.accepted){
    primary=null;
    for(const s of signalDefs)autoSignal(s.id,'Not analyzed because no uploaded report adequately represents this company.','Not found in provided sources.',[],s.keys);
  ...
  const rp=primary.profile.regulation;
  const regEv=regulationText();
  const themes=themeList(rp?.text||'');
  const regValue=rp?.evidence
    ? 'IBISWorld rates Regulation & Policy as '+(rp.rating?(rp.rating.level.toUpperCase()+' and '+rp.rating.trend.toUpperCase()):'not explicitly rated in the parsed text')+'. Key source-supported pressure areas include '+(themes.length?themes.join(', '):'the regulatory topics described in the cited section')+'.'
    : 'Not found in provided sources.';
  autoSignal('regulation',regValue,rp?.evidence?citation(rp.evidence):'',regEv,signalDefs.find(s=>s.id==='regulation').keys);

  const pw=primary.profile.power;
  const supEv=supplierPowerText();
  let supValue='Not found in provided sources.';
  if(pw?.evidence){
    const bits=[];
    if(/NVIDIA.*92\.0%/i.test(pw.supplierText))bits.push('NVIDIA’s concentrated GPU position');
    if(/Apple and Google|Apple.*Google/i.test(pw.supplierText))bits.push('Apple/Google mobile-platform gatekeeping');
    if(/AWS.*Azure.*GCP|Azure.*GCP/i.test(pw.supplierText))bits.push('competitive cloud-provider alternatives');
    if(/switching costs|limited alternatives|Dell|HP|Cisco/i.test(pw.supplierText))bits.push('hardware ecosystem switching costs and limited specialized-hardware alternatives');
    supValue='Supplier power is '+(pw.supplier?(pw.supplier.level.toUpperCase()+' and '+pw.supplier.trend.toUpperCase()):'described in the report')+'. '+
      (bits.length?('The report specifically identifies '+bits.join(', ')+'. '):'')+
      'Analyst interpretation: these dependencies create operational fragility only where the source shows concentration or switching costs; dependency alone is not treated as high supplier power.';
  }
  autoSignal('supply',supValue,pw?.evidence?citation(pw.evidence):'',supEv,signalDefs.find(s=>s.id==='supply').keys);

  const mm=parseMajorMarkets();
  const custEv=buyerPowerText();
  let custValue='Not found in provided sources.',custSrc='';
  if(mm){
    const sorted=[...mm.pairs].sort((a,b)=>Number(b.share)-Number(a.share));
    const top3=sorted.slice(0,3).reduce((s,x)=>s+Number(x.share),0);
    custValue='2026 demand is distributed across '+mm.pairs.length+' reported customer markets: '+
      mm.pairs.map(x=>x.segment+' '+x.share+'%').join('; ')+'. The top three verticals account for '+top3.toFixed(1)+'% of industry revenue. '+
      (pw?.buyer?('IBISWorld rates buyer power as '+pw.buyer.level.toUpperCase()+' and '+pw.buyer.trend.toUpperCase()+'. '):'')+
      'Analyst interpretation: vertical revenue can be concentrated even while the underlying buyer population is fragmented.';
    custSrc=citation(mm.e);
  }else if(pw?.evidence){
    custValue=(pw.buyer?('Buyer power is '+pw.buyer.level.toUpperCase()+' and '+pw.buyer.trend.toUpperCase()+'. '):'')+
      (bestSentence(pw.buyerText,['fragmented','buyers','customers','businesses'])||'The buyer structure is described in the cited section.');
    custSrc=citation(pw.evidence);
  }
  autoSignal('customers',custValue,custSrc,custEv,signalDefs.find(s=>s.id==='customers').keys);

  const outlook=primary.profile.outlook?.text||'';
  const tr=trendText();
  let trValue='Not found in provided sources.',trSrc=tr[0]?citation(tr[0]):'';
  if(/free and open-source alternatives/i.test(outlook)&&/AI|artificial intelligence|generative/i.test(outlook)){
    trValue='Analyst interpretation — biggest five-year trend: AI-driven and generative-AI-enabled workflows. The outlook describes deeper AI automation, asset generation, intelligent assistants and AI-supported collaboration as a central direction for the industry.';
  }else if(/AI|artificial intelligence/i.test(outlook)){
    trValue='Analyst interpretation — biggest five-year trend: broader AI adoption, which is increasing demand for cleaner data, scalable infrastructure, specialized talent and AI-enabled product capabilities.';
  }else if(/cybersecurity|zero-trust/i.test(outlook)){
    trValue='Analyst interpretation — biggest five-year trend: rising investment in cybersecurity and zero-trust capabilities as software vendors compete on trust, resilience and compliance.';
  }else if(tr.length){
    trValue=evidenceSnippet(tr[0],signalDefs.find(s=>s.id==='trend').keys);
  }
  autoSignal('trend',trValue,trSrc,tr,signalDefs.find(s=>s.id==='trend').keys);

  const th=threatText();
  let thValue='Not found in provided sources.',thSrc=th[0]?citation(th[0]):'';
  if(/threat posed by free and open-source alternatives|free and open-source alternatives.*intensif/i.test(outlook)){
    thValue='Analyst interpretation — biggest five-year threat: increasing competition from free and open-source alternatives, which the report expects to narrow functional gaps and pressure proprietary vendors’ pricing and differentiation.';
  }else if(/market saturation|saturated/i.test(outlook)){
    thValue='Analyst interpretation — biggest five-year threat: market saturation, which the report expects to slow revenue growth, make new customer acquisition harder and intensify reliance on M&A, bundling and cross-selling.';
  }else if(/cyber attacks|cybersecurity/i.test(outlook)){
    thValue='Analyst interpretation — biggest five-year threat: escalating cybersecurity risk and the cost of maintaining trusted, resilient enterprise platforms.';
  }else if(th.length){
    thValue=evidenceSnippet(th[0],signalDefs.find(s=>s.id==='threat').keys);
  }
  autoSignal('threat',thValue,thSrc,th,signalDefs.find(s=>s.id==='threat').keys);
  ...
  const ce=(primary.profile?.shares||[]).map(x=>x.evidence).filter(Boolean).slice(0,4);
  renderEvidence('compEvidence',ce,['market share','company'],e=>{
    const i=[0,1,2].find(i=>!$('compSrc'+i)?.value);
    if(i!==undefined)$('compSrc'+i).value=citation(e);
    updateAudit();
  });

  refreshMissing();
  $('runStatus').textContent='Done. “'+primary.title+'” selected as the primary report. All industry statistics below come from that report unless explicitly marked supplemental.';
  updateAudit();
}
...
renderFiles();
+rev[1]+(rev[2].toLowerCase().startsWith('b')?'bn':'m')):'',
    historic:rates[0]?{start:rates[0][1],end:rates[0][2],rate:rates[0][3]}:null,
    forecast:rates[1]?{start:rates[1][1],end:rates[1][2],rate:rates[1][3]}:null,
    profit:profit?('
function companyMetrics(d,company){
...
$('json').onclick=()=>download(`${($('company').value||'company').replace(/\W+/g,'-').toLowerCase()}-industry-brief.json`,'application/json',JSON.stringify(currentData(),null,2));
renderFiles();
+profit[1]+(profit[2].toLowerCase().startsWith('b')?'bn':'m')):'',
...
$('json').onclick=()=>download(`${($('company').value||'company').replace(/\W+/g,'-').toLowerCase()}-industry-brief.json`,'application/json',JSON.stringify(currentData(),null,2));
renderFiles();
+wages[1]+(wages[2].toLowerCase().startsWith('b')?'bn':'m')):'',
    evidence:ev(d,p,'At a Glance')
  };
}
function extractMajorMarketsProfile(d){
  const p=d.pages.find(p=>/Major Markets Segmentation/i.test(flat(p.text)));
  if(!p)return {markets:[],evidence:null};
  const f=flat(p.text), s=f.indexOf('Major Markets Segmentation'), tail=f.slice(s,Math.min(f.length,s+1800));
  const markets=[];
  for(const m of tail.matchAll(/([A-Z][A-Za-z0-9,&'’ \/-]{2,80})\s*\(\$?[\d,.]+\s*(?:bn|m|million|billion)?\)\s*(\d+(?:\.\d+)?)%/g)){
    let name=clean(m[1]).replace(/^Industry revenue.*?markets\s*/i,'').trim();
    if(name.length>2&&!/Source: IBISWorld/i.test(name))markets.push({name:name,share:Number(m[2])});
  }
  return {markets:markets.slice(0,8),evidence:ev(d,p,'Major Markets',tail)};
}
function extractPower(d){
  const p=d.pages.find(p=>/Buyers:/i.test(flat(p.text))&&/Suppliers:/i.test(flat(p.text)));
  if(!p)return {buyer:null,supplier:null,evidence:null,buyerText:'',supplierText:''};
  const f=flat(p.text);
  const bm=f.match(/(Low|Moderate|High)\s+(Steady|Increasing|Decreasing)\s+Buyers:/i);
  const sm=f.match(/(Low|Moderate|High)\s+(Steady|Increasing|Decreasing)\s+Suppliers:/i);
  const bStart=f.search(/Buyers:/i), sStart=f.search(/Suppliers:/i);
  const buyerText=bStart>=0?f.slice(bStart,sStart>bStart?sStart:Math.min(f.length,bStart+1800)):'';
  const supplierText=sStart>=0?f.slice(sStart,Math.min(f.length,sStart+1800)):'';
  return {buyer:bm?{level:bm[1],trend:bm[2]}:null,supplier:sm?{level:sm[1],trend:sm[2]}:null,buyerText:buyerText,supplierText:supplierText,evidence:ev(d,p,'Buyer & Supplier Power',f)};
}
function extractRegulationProfile(d){
  const p=d.pages.find(p=>/Regulation\s*&\s*Policy/i.test(flat(p.text))&&/What regulations impact the industry/i.test(flat(p.text)));
  if(!p)return {rating:null,text:'',evidence:null};
  const f=flat(p.text);
  const m=f.match(/Regulation\s*&\s*Policy\s+(Low|Moderate|High)\s+(Steady|Increasing|Decreasing)/i);
  const text=blockBetween(f,/Regulation\s*&\s*Policy(?:\s+(?:Low|Moderate|High)\s+(?:Steady|Increasing|Decreasing))?/i,/Assistance\s+(?:Low|Moderate|High)/i);
  return {rating:m?{level:m[1],trend:m[2]}:null,text:text,evidence:ev(d,p,'Regulation & Policy',text||f)};
}
function extractOutlookProfile(d){
  const i=d.pages.findIndex(p=>/What.?s driving the industry outlook/i.test(flat(p.text)));
  if(i<0)return {text:'',pages:[]};
  const pages=d.pages.slice(i,Math.min(d.pages.length,i+3));
  return {text:pages.map(p=>flat(p.text)).join(' '),pages:pages};
}
function buildProfile(d){
  const companies=extractCompanyList(d);
  const shares=extractMarketShares(d,companies);
  const about=d.pages.find(p=>/Definition/i.test(flat(p.text))&&/What.?s Included/i.test(flat(p.text)));
  const aboutText=about?flat(about.text):'';
  const nm=aboutText.match(/2022\s+(\d{6})\s*[–—-]\s*([A-Za-z][A-Za-z &(),.-]+)/);
  const def=blockBetween(aboutText,/Definition/i,/Codes/i);
  return {companies:companies,shares:shares,atAGlance:extractAtAGlance(d),majorMarkets:extractMajorMarketsProfile(d),power:extractPower(d),regulation:extractRegulationProfile(d),outlook:extractOutlookProfile(d),naics:nm?{code:nm[1],title:nm[2].trim(),evidence:ev(d,about,'About This Industry / Codes',around(aboutText,nm.index,500))}:null,definition:def,aboutEvidence:about?ev(d,about,'About This Industry',aboutText):null};
}
function companyPresence(d,company){
  const pr=d.profile||buildProfile(d), target=company.toLowerCase();
  const sh=pr.shares.find(x=>x.name.toLowerCase()===target);
  const listed=pr.companies.some(x=>x.toLowerCase()===target);
  let mentions=0, detail=null;
  for(const p of d.pages){
    const f=flat(p.text);
    mentions+=(f.match(new RegExp("\\b"+rxesc(company)+"\\b","gi"))||[]).length;
    if(!detail && /Company Total Revenue/i.test(f) && new RegExp("\\b"+rxesc(company)+"\\b","i").test(f.slice(0,900)))detail=ev(d,p,'Company Details',f.slice(0,1500));
  }
  const score=(sh?100+Math.min(sh.upper*2,60):0)+(detail?45:0)+(listed?20:0)+Math.min(mentions,20);
  return {share:sh,listed:listed,mentions:mentions,detail:detail,score:score};
}
function formatYearEnd(start,end){return String(end).length===2?String(start).slice(0,2)+end:end}
function themeList(text){
  const t=text.toLowerCase(), a=[];
  if(/copyright|intellectual property|\bip\b/.test(t))a.push('copyright/IP');
  if(/privacy|ccpa|cpra|gdpr|data protection/.test(t))a.push('data privacy and security');
  if(/coppa|child|parental consent/.test(t))a.push('child-data privacy');
  if(/antitrust|merger|acquisition/.test(t))a.push('antitrust/M&A scrutiny');
  if(/automated decision|artificial intelligence|\bai\b/.test(t))a.push('AI governance');
  if(/cybersecurity|security audit/.test(t)&&!a.includes('data privacy and security'))a.push('cybersecurity');
  return [...new Set(a)];
}
function bestSentence(text,terms){
  const s=flat(text).split(/(?<=[.!?])\s+(?=[A-Z0-9])/); let best='',score=-1;
  for(const x of s){const l=x.toLowerCase();let q=0;for(const t of terms)if(l.includes(t.toLowerCase()))q+=3;if(/\d/.test(x))q++;if(q>score){score=q;best=x}}
  return best;
}
